...
print("femaleeeeeeeeeee")


feedback_csv_path = "/root/piper/src/python/streamlit_output/model_error_feedback.csv"
last_processed_json = "/root/piper/src/python/feedback_pipline/last_processed_run_female.json"
# Source folder to backup
src_dir = "/root/piper/trained_models/female_2500_base_10k"
# Create a backup folder with a timestamp
backup_dir = f"/root/piper/src/python/feedback_pipline/backup/female_2500_base_10k_backup_{timestamp}"
# Update the global error feedback CSV using the feedback DataFrame
global_error_csv = "/root/piper/src/python/feedback_pipline/Global_feeback_count_female/global_error_feedback.csv"
output_csv_dir = f"/root/piper/src/python/feedback_pipline/generated_sentences_female/"
output_csv_path = f"/root/piper/src/python/feedback_pipline/generated_sentences_female/Generated_sentences_{timestamp}.csv"
output_dir = f"/root/piper/src/python/feedback_pipline/Audio_data_female/base_audio_files_{timestamp}"
metadata_csv = f"/root/piper/src/python/feedback_pipline/metadata_female/meta_{timestamp}/metadata.csv"
readydata_dir = f"/root/piper/src/python/feedback_pipline/readydata_for_training_female/process_{timestamp}"
output_dir_preprocess = os.path.expanduser(f'/root/piper/src/python/feedback_pipline/pipeline_training_female/model_{timestamp}')
...
df_feedback = df_feedback[df_feedback["Model"].str.strip() == "Sadaa-e-Niswan"]
...
if max_feedback_time > last_processed_time:
    ...
    # Filter for rows where the Model is "Celestia X"
    df_words = df_words[df_words["Model"].str.strip() == "Sadaa-e-Niswan"]
    ...
    if ckpt_files:
        ...
        if match:
            ...
            print("New Epoch Number :", new_epoch)
        else:
